...
- Exposes only path-relative unified discovery endpoint: /.well-known/authorization_servers/mcp
"""
...
class ResourceServerSettings(BaseSettings):
    """Settings for the multi-protocol MCP Resource Server (path-only discovery)."""
...
def create_multiprotocol_resource_server(settings: ResourceServerSettings) -> Starlette:
    """Create Starlette app with MultiProtocolAuthBackend, PRM (no mcp_auth_protocols) and path-only discovery."""
    ...
    fastmcp = FastMCP(
        name="MCP Resource Server (multiprotocol, path-only discovery)",
    ...
    # Path-relative unified discovery endpoint (Way B: /.well-known/authorization_servers/mcp)
    ...
    # NOTE: path-only variant intentionally does NOT add root-based unified discovery
    # (no /.well-known/authorization_servers route)
...
def main(
    ...
    """Run the multi-protocol MCP Resource Server (path-only discovery)."""
    ...
    logger.info("Multi-protocol RS (path-only discovery) running on %s", settings.server_url)
