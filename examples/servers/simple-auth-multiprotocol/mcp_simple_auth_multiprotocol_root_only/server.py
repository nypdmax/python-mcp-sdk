...
- Exposes only root unified discovery endpoint: /.well-known/authorization_servers
"""
...
class ResourceServerSettings(BaseSettings):
    """Settings for the multi-protocol MCP Resource Server (root-only discovery)."""
...
def create_multiprotocol_resource_server(settings: ResourceServerSettings) -> Starlette:
    """Create Starlette app with MultiProtocolAuthBackend, PRM (no mcp_auth_protocols) and root-only discovery."""
    ...
    fastmcp = FastMCP(
        name="MCP Resource Server (multiprotocol, root-only discovery)",
    ...
    # PRM without MCP extensions: provide RFC 9728 authorization_servers/scopes only.
    # This allows OAuth to locate the AS without letting clients discover protocols via PRM mcp_auth_protocols.
    routes.extend(
        create_protected_resource_routes(
    ...
    # Root-based unified discovery endpoint only (/.well-known/authorization_servers)
    routes.extend(
        create_authorization_servers_discovery_routes(
    ...
    # NOTE: root-only variant intentionally does NOT add path-relative unified discovery
    # (no /.well-known/authorization_servers/mcp route)
...
def main(
    ...
    """Run the multi-protocol MCP Resource Server (root-only discovery)."""
    ...
    logger.info("Multi-protocol RS (root-only discovery) running on %s", settings.server_url)
